...
class KeyboardBuilder(Generic[ButtonType], ABC):
    ...
    def _validate_markup(self, markup: List[List[ButtonType]]) -> bool:
        ...
        if count > MAX_BUTTONS:
            raise ValueError(
                f"Too much buttons detected Max allowed count - {MAX_BUTTONS}"
            )
        return True
    ...
    def adjust(
        self, *sizes: int, repeat: bool = False
    ) -> "KeyboardBuilder[ButtonType]":
        """
        ...
        sizes_iter = (
            repeat_all(validated_sizes) if repeat else repeat_last(validated_sizes)
        )
    ...
    def attach(
        self, builder: "KeyboardBuilder[ButtonType]"
    ) -> "KeyboardBuilder[ButtonType]":
        if not isinstance(builder, KeyboardBuilder):
            raise ValueError(
                f"Only KeyboardBuilder can be attached, not {type(builder).__name__}"
            )
...
class InlineKeyboardBuilder(KeyboardBuilder[InlineKeyboardButton]):
    ...
    def __init__(
        self, markup: Optional[List[List[InlineKeyboardButton]]] = None
    ) -> None:
